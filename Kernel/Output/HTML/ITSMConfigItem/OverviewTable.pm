...
package Kernel::Output::HTML::ITSMConfigItem::OverviewTable;
...
sub Run {
	...
	# COMPLEMENTO: Get Param Object Again
	my $ParamObject = $Kernel::OM->Get('Kernel::System::Web::Request');
	# We need get another parameter for sorting to not conflict with SortBy. We also store the prefered sort order of the user	
	$Param{SortByAfter} = $ParamObject->GetParam( Param => 'SortByAfter' )||$UserPreferences{'ITSMCMDBSortByAfter'.$Param{Filter}} || 'Number';
	$Param{OrderBy} = $ParamObject->GetParam( Param => 'OrderBy' )||$UserPreferences{'ITSMCMDBOrderByAfter'.$Param{Filter}} || 'Down';



    $UserObject->SetPreferences(
        UserID => $Self->{UserID},
        Key    => 'ITSMCMDBSortByAfter'.$Param{Filter},
        Value  => $Param{SortByAfter},
    );
    $UserObject->SetPreferences(
        UserID => $Self->{UserID},
        Key    => 'ITSMCMDBOrderByAfter'.$Param{Filter},
        Value  => $Param{OrderBy},
    );
...
END
    ...
    if (@ShowColumns) {
        ...
        for my $Column (@ShowColumns) {
            ...
            if ( $Param{SortByAfter} ) {
                $Param{SortByAfter} = ( $Param{SortByAfter} eq 'InciStateID' )
                    ? 'CurInciState'
                    : ( $Param{SortByAfter} eq 'DeplStateID' ) ? 'CurDeplState'
                    : ( $Param{SortByAfter} eq 'ClassID' )     ? 'Class'
                    : ( $Param{SortByAfter} eq 'ChangeTime' )  ? 'LastChanged'
                    :                                       $Param{SortByAfter};
            }

            # set the correct Set CSS class and order by link
            if ( $Param{SortByAfter} && ( $Param{SortByAfter} eq $Column ) ) {
            ...
            # Deployment State for editing the column
            if($Column eq 'CurDeplState'){
            	my @DeploymentStatesListArray = sort keys %{$DeploymentStatesList};
            	my $i=0;
                for my $ItemID ( @DeploymentStatesListArray ) {
                	my $Comma=',';
		            if(  $i == $#DeploymentStatesListArray  ) {
						$Comma='';
					}
					$i++;
				    $LayoutObject->Block(
				        ...
				        Data => {
							ID => $ItemID,
							State => $DeploymentStatesList->{$ItemID},
							Comma => $Comma
				        },
				    );
                }
...
#########################
				...
				# COMPLEMENTO EDITABLE COLUMN
				if($Column =~ /::/){
				    $LayoutObject->Block(
				        Name => 'EditableColumn',
				        Data => {},
				    );
				    if($ExtendedVersionData->{$Column}->{InputDefinition}->{Type} eq 'Text'){
						$LayoutObject->Block(
							Name => 'EditableColumnText',
							Data => {
							    Name => $Column,
							},
						);
					} elsif($ExtendedVersionData->{$Column}->{InputDefinition}->{Type} eq 'GeneralCatalog'){

						# get list of deployment states
						my $GeneralCatalogList = $GeneralCatalogObject->ItemList(
							Class => $ExtendedVersionData->{$Column}->{InputDefinition}->{Class},
						);
						
						my %GeneralCatalogHash = %{$GeneralCatalogList};
						
						my $GeneralCatalogString = join(",", map { '"'.$_.'":"'.$LayoutObject->{LanguageObject}->Translate($GeneralCatalogHash{$_}).'"' } keys %GeneralCatalogHash);
						$LayoutObject->Block(
							Name => 'EditableColumnGeneralCatalog',
							Data => {
							    Name => $Column,
							    GeneralCatalog => $GeneralCatalogString
							},
						);
					}
...
####################################################### ORDER Config Item for other attributes
        CONFIGITEMID:
        for my $ConfigItemID (@ConfigItemIDs) {
			...
			$DataForOrdering{$ConfigItemID}->{Name} = $ConfigItem->{Name};
			
			if ($CommonOrderByTable{$Param{SortByAfter}}){
				$DataForOrdering{$ConfigItemID}->{Value} = $ConfigItem->{$Param{SortByAfter}} || '';
			} else {
		        $DataForOrdering{$ConfigItemID}->{Value} = $Kernel::OM->Get('Kernel::System::HTMLUtils')
		                        ->ToAscii( String => $ExtendedVersionData->{"$Param{SortByAfter}"}->{Value} ) || '';
            }
        }
        
		my @OrderedConfigItemIDs = sort {
										   ($DataForOrdering{$a}->{Value} <=> $DataForOrdering{$b}->{Value})
										|| (lc($DataForOrdering{$a}->{Value}) cmp lc($DataForOrdering{$b}->{Value}))
										# keep some sort of same result when everything is similar
										|| (lc($DataForOrdering{$a}->{Name})  cmp lc($DataForOrdering{$b}->{Name}))
										|| ($a   <=>  $b)
		} keys(%DataForOrdering);
		
        if ($Param{'OrderBy'} eq 'Down') {
        	@OrderedConfigItemIDs=reverse @OrderedConfigItemIDs;
        }
		...
		$Counter = 0;
        CONFIGITEMID:
        for my $ConfigItemID (@OrderedConfigItemIDs) {
            $Counter++;
            if (
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'AgentITSMConfigItemOverviewTable',
        Data         => {
            %Param,
            Type         => $Self->{ViewType},
